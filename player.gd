...
func _physics_process(delta: float) -> void:
	...
	Global.mouse_location = get_local_mouse_position()
	if Global.mouse_location < Vector2(0, 0):
		$Sprite.flip_h = false
	elif Global.mouse_location > Vector2(0, 0):
		$Sprite.flip_h = true
	if $Sprite.flip_h:
	...
	#LOCALIZAÇÂO DO MOUSE
	#print("X: ",Global.mouse_location[0], " | Y: ", Global.mouse_location[1])
